...
numericLines <- lapplyPartitionsWithIndex(file,
                       function(split, part) {
                         ...
                         # unlist transforms list to vector
...
featureLabels <- cache(lapplyPartition(
    ...
    function(part) {
      ...
      # vector of ones
      ones <- rep(1, nrow(mat))
      # t() is transpose, value() unwraps broadcast variable, % means infix operator
      features <- cos(
        mat %*% t(value(broadcastW)) + (matrix(ncol=1, data=ones) %*% t(value(broadcastB))))
      # matrix of ones
      onesMat <- Matrix(ones)
      # cbind is horizontal concatenation
